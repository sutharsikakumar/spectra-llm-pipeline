...
OUT_FILE  = ROOT / "results" / "mos2_peaks_analysis_llama.json"
SUMMARY_FILE = ROOT / "results" / "mos2_summary_llama.json"

LLAMA_API_KEY = os.environ.get("LLAMA_API_KEY")
if not LLAMA_API_KEY:
    raise EnvironmentError("LLAMA_API_KEY environment variable is required")

LLAMA_API_URL = "https://api.llama.com/v1/chat/completions"
LLAMA_MODEL = "llama-3-70b"
TEMPERATURE  = 0.3
MAX_RETRIES  = 5
MAX_TOKENS   = 5000
...
def call_llama(messages, model=LLAMA_MODEL, temperature=TEMPERATURE, max_tokens=MAX_TOKENS):
    headers = {
    ...
    for attempt in range(1, MAX_RETRIES + 1):
        try:
            ...
            if attempt < MAX_RETRIES:
                ...
                continue
            raise ValueError(f"Llama returned invalid JSON after {MAX_RETRIES} attempts:\n{content}") from err
    raise RuntimeError(f"Llama request failed after {MAX_RETRIES} attempts") from last_error
...
def analyze(peaks: List[dict]) -> List[dict]:
    analyses: List[dict] = []
    print(f"Analyzing {len(peaks)} peaks with Llama...")
    for i, peak in enumerate(peaks, 1):
        print(f"Processing peak {i}/{len(peaks)} at {peak['position']:.2f} cm⁻¹...")
        try:
            result = ask_llama(build_prompt(peak))
...
def generate_material_summary_text(material: str, confidence: str, supporting_peaks: int,
                                 total_peaks: int, all_assignments) -> str:
...
def main() -> None:
    print("Raman Peak Analysis with Llama")
    print("=" * 50)
