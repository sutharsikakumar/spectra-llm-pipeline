...
OUT_FILE  = ROOT / "results" / "mos2_peaks_analysis_deepseek.json"
SUMMARY_FILE = ROOT / "results" / "mos2_summary_deepseek.json"
...
def call_deepseek(messages, model=DEEPSEEK_MODEL, temperature=TEMPERATURE, max_tokens=MAX_TOKENS):
    headers = {
    ...
    for attempt in range(1, MAX_RETRIES + 1):
        try:
            ...
            if attempt < MAX_RETRIES:
                ...
                continue
            raise ValueError(f"DeepSeek returned invalid JSON after {MAX_RETRIES} attempts:\n{content}") from err
    raise RuntimeError(f"DeepSeek request failed after {MAX_RETRIES} attempts") from last_error
...
def analyze(peaks: List[dict]) -> List[dict]:
    analyses: List[dict] = []
    print(f"Analyzing {len(peaks)} peaks with DeepSeek...")
    for i, peak in enumerate(peaks, 1):
        print(f"Processing peak {i}/{len(peaks)} at {peak['position']:.2f} cm⁻¹...")
        try:
            result = ask_deepseek(build_prompt(peak))
...
def main() -> None:
    print("Raman Peak Analysis with DeepSeek")
    print("=" * 50)
