...
OUT_FILE  = ROOT / "results" / "mos2_peaks_analysis_grok.json"
SUMMARY_FILE = ROOT / "results" / "mos2_summary_grok.json"

GROK_API_KEY = os.environ.get("GROK_API_KEY")
if not GROK_API_KEY:
    raise EnvironmentError("GROK_API_KEY environment variable is required")
...
def call_grok(messages, model=GROK_MODEL, temperature=TEMPERATURE, max_tokens=MAX_TOKENS):
    headers = {
    ...
    for attempt in range(1, MAX_RETRIES + 1):
        try:
            ...
            if attempt < MAX_RETRIES:
                ...
                continue
            raise ValueError(f"Grok returned invalid JSON after {MAX_RETRIES} attempts:\n{content}") from err
    raise RuntimeError(f"Grok request failed after {MAX_RETRIES} attempts") from last_error
...
def analyze(peaks: List[dict]) -> List[dict]:
    analyses: List[dict] = []
    print(f"Analyzing {len(peaks)} peaks with Grok...")
    for i, peak in enumerate(peaks, 1):
        print(f"Processing peak {i}/{len(peaks)} at {peak['position']:.2f} cm⁻¹...")
        try:
            result = ask_grok(build_prompt(peak))
...
def main() -> None:
    print("Raman Peak Analysis with Grok")
    print("=" * 50)
